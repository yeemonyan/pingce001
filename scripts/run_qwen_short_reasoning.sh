...
CONFIG_PATH="${CONFIG_PATH:-configs/train_qwen_short_reasoning.yaml}"
RUN_TRAIN="${RUN_TRAIN:-1}"
PROMPTS_PATH="${PROMPTS_PATH:-configs/system_prompts_v2.yaml}"
MODEL_PATH="${MODEL_PATH:-models/Qwen2.5-7B-Instruct}"
TRAIN_OUTPUT_DIR="${TRAIN_OUTPUT_DIR:-checkpoints/qwen2p5_7b_lora_reasoning_short_seed2026}"
PRED_PATH="${PRED_PATH:-outputs/dev_lora_reasoning_short_predictions.jsonl}"
EVAL_PATH="${EVAL_PATH:-outputs/dev_lora_reasoning_short_eval.json}"

echo "[1/5] Build SFT data variants"
"$PYTHON_BIN" scripts/build_sft_data.py --output-dir outputs --report outputs/sft_data_report.json
...
echo "[3/5] Train Qwen short-reasoning adapter"
...
echo "qwen_short_reasoning_ready=$EVAL_PATH"
